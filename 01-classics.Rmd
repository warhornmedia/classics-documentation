...
.github folder
: contains the Github Actions workflow file that rebuilds the book automatically every time a new commit is pushed to Github. You shouldn't need to mess with this folder. In fact, you normally won't see it at all if you are working on a Mac.

.gitignore
: Specifies which files are not to be tracked in git. In particular, we only want to track book source, not output files (_book), files related to RStudio, or external files used in the build process (classics-template-files). This file already has the right settings. You shouldn't need to mess with this file. In fact, you normally won't see it at all if you are working on a Mac.
...
2. Make sure every .Rmd file starts with a # or ##.
