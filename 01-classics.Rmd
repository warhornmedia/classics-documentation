...
Endnotes can be created simply by creating a separate .Rmd file[^5] for them, and giving them named IDs and then [linking to them](#linking-to-an-endnote) as references from the body of the main text, and creating backlinks. In this way, they are not special links, and they are compatible with footnotes. The end of this sentence has an endnote to demonstrate what an endnote normally looks like. ([1](#en.01){#en.01.backlink})

**NOTE**: Since endnotes are simply internal links, they are not able to be styled differently (such as being in superscript) by default. If need be, there may be ways around this limitation.
