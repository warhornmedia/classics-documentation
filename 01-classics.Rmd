...
Endnotes can be created simply by creating a separate file for them, and giving them named IDs and then linking to them as references from the body of the main text. In this way, they are not special links, and they are compatible with footnotes. Customarily we have used the lowest level of header for the endnote title, unnumbered, with an explicit ID like so:
...
To link to the endnote from within the text, simply follow the instructions for references (internal links) [above](#references). Here is an example of what it looks like to link to an endnote in the markdown:

```markdown
Sentence ends with a linked endnote in parentheses.([2](#en.p1.02))
```
