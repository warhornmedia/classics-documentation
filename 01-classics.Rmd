...
### Files in a book project

If the name of a field is **bold** below, that means you need to modify that field when you create a new book from the template.
...
[^3]: Supposedly the first file must begin with either a chapter ("# Chapter name") *or* a section ("## Section title"). However, it appears that it must be a chapter, as starting with a section causes an error. Latter files may start with a section, instead if necessary to split the book into appropriate-length pages.
...
Note that the numbers at the beginning of each filename are *only* used to get the files into the proper order in the book. It is handy to prevent confusion during editing if they can correspond to the chapter number, but it is not necessary.
