...
\NormalTok{| \textbackslash{} \textbackslash{} \textbackslash{} \textbackslash{} The day of woe, the watchful night,}
...
Precede each line of poetry with a \textbar{} and a space. (This makes a \protect\hyperlink{https:ux2fux2fpandoc.orgux2fMANUAL.htmlux5cux23line-blocks}{line block} so there are line breaks between consecutive lines. We could also use a backslashe (\textbackslash) at the end of each line to do the same.)

If a line of poetry should appear indented further than the other lines in its final form, precede the text of the indented line with non-breaking spaces, which are entered as spaces preceded by backslashes. If you are using the line block method, spaces at the start of the line will be respected, regardless of whether you add the backslash. Both methods are shown above.
